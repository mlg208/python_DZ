...
def quick_sort(the_list):
    if len(the_list) <= 1:
        return the_list
    pivot = the_list[0]
    first_list = list(filter(lambda x: x < pivot, the_list))
    second_list = list(filter(lambda x: x == pivot, the_list))
    third_list = list(filter(lambda x: x > pivot, the_list))
    result = quick_sort(first_list) + second_list + quick_sort(third_list)
    return result
